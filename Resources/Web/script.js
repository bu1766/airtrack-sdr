...
jQuery('#selected_altitude_geom1')
    ...
    */
    ...
    if (routeApiUrl) {
        const localRoutePanel = routeApiUrl == "/api/routes";
        // This local build always exposes the route panel. Ignore an older
        // browser preference that may have disabled route lookups.
        if (localRoutePanel) {
            loStore['useRouteAPI'] = 'true';
            useRouteAPI = true;
            jQuery('#routeRow').removeClass('hidden').show();
        }
...
function refreshSelected() {
    ...
    const aircraftModel = selected.typeLong || selected.icaoType || 'n/a';
    jQuery('#selected_aircraft_model').updateText(aircraftModel);
    jQuery('#selected_aircraft_code').updateText(
        selected.typeLong && selected.icaoType ? selected.icaoType : ''
    );
    ...
    if (useRouteAPI) {
        if (selected.routeString) {
            ...
            const routeCodes = selected.routeString
                .replace(/^\?\?\s*/, '')
                .split(/\s*-\s*/)
                .filter(Boolean);
            const routeCities = (selected.routeVerbose || '')
                .split(/\s*-\s*/)
                .filter(Boolean);
            jQuery('#selected_route_from').updateText(routeCodes[0] || 'n/a');
            jQuery('#selected_route_to').updateText(routeCodes.at(-1) || 'n/a');
            jQuery('#selected_route_from_city').updateText(routeCities[0] || '');
            jQuery('#selected_route_to_city').updateText(routeCities.at(-1) || '');
        } else {
            jQuery('#selected_route').updateText('n/a');
            jQuery('#selected_route_from').updateText('n/a');
            jQuery('#selected_route_to').updateText('n/a');
            jQuery('#selected_route_from_city').updateText('');
            jQuery('#selected_route_to_city').updateText('');
        }
    }
